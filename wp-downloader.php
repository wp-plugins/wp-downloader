...
function wpd_plugin_action_links($links, $file){
	$settings_link = '<a href="'.wp_nonce_url(admin_url('?wpd=plugin&object='.$file), 'wpd-download').'">'.__('Download').'</a>';
...
function wpd_theme_action_links($links, $theme){
	$settings_link = '<a href="'.wp_nonce_url(admin_url('?wpd=theme&object='.$theme->get_stylesheet()), 'wpd-download').'">'.__('Download').'</a>';
	array_push($links, $settings_link); 
